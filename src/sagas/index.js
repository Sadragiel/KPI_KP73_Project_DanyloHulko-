...
function* fetchCucold(actionObj) {

  const url = `https://dog.ceo/api/breeds/image/random`;

  const json = yield fetch(url).then(response => response.json(), );
  yield [put({ type: "CUCOLD_RECEIVED", cucoldActionData: json.message })];
}

function* actionCucoldWatcher() {
  yield takeLatest('GET_CUCOLD', fetchCucold)
}
